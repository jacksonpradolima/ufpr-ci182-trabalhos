...
data_inicio = '15 Sep 2019' #Escolha a Data de início para a criação da base de dados do par de moedas escolhido. Deve ser entre aspas e dias, meses e anos espaçados e os meses abreviados com inicial maiuscula
lista_intervalo = ['4h'] #Intervalo de tempo, opções encontradas no binsizes acima
...
while True:
    if (tempolocal[3] - 1) % 4 == 0 and tempolocal[4] == 0 and tempolocal[5] == 10: #A análise de compra é gerada logo após o horário de fechamento(Close) do candle no intervalo escolhido(1d aqui)
            ...
            for simbolo in binance_simbolos:
                ...
                for i in range(0,len(base)):
                    if Comprar_Vender[i] > 0:
                        Comprar_Vender[i] = Comprar_Vender[i] / 25
                ...
                if Comprou_Vendeu < 14: #Limite de compra de 14 moedas
                    if (os.path.exists('%s.txt' %simbolo) == False  #Caso o arquivo não exista (não foi comprada), a moeda pode estar sendo comprada
                    and Comprar_Vender[final_grafico - inicio_grafico - 2] > 0): #Ultimo candle de fechamento (do intervalo) com indicação de barra verde
                        ...
                        print('Hora {}, dia {}, mês {}, ano {}'.format(tempolocal[3], tempolocal[2], tempolocal[1], tempolocal[0])) #Momento exato que a moeda foi comprada
                        
                        quantidade = int(0.01 / Close[len(base) - 2]) #Quantidade de moedas para comprar usando 0.01BTC, porém em formato inteiro, afim de evitar conflitos nas ordens
                ...
                if os.path.exists('%s.txt' %simbolo) == True: #Caso o arquivo exista(indica que a moeda foi comprada e agora pode estar sendo vendida)
                    if Comprar_Vender[final_grafico - inicio_grafico - 2] < 0: #Ultimo candle de fechamento (do intervalo) com indicação de barra vermelha
